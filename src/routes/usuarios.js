 ...
 * El Id se obtine de los parametros de la URL (req.paramas) y se utiliza para buscar el usuario en la base de datos mediante 'UsuariofindById(id)'
 */
...
router.put('/actualizarUsuario/:id', (req, res) =>{
    const{id} = req.params;
    const {nombreUsuario, nombre, password, huella} = req.body;
    Usuario.updateOne({ _id: id }, { $set: {nombreUsuario, nombre, password, huella } })
    .then((data) => res.json(data))
    .catch((error) => res.json({message: error}));
});
...
router.get('/obtenerUsuarioByNombreUsuario/:nombreUsuario', (req, res) =>{
    ...
    Usuario.findOne({nombreUsuario: nombre})
    .then((data) => res.json(data))
    .catch((error) => res.json({message: error}));
});
